...
public class BoardTestSuite {
    public Board prepareTestData(){
        ...
        //tasks
        Task task1 = new Task("Microservice for taking temperature", "Write and test the microservice taking the temperaure from external service",
    ...
    @Test
    public void testAddTaskListFinUsersTasks(){
        //Given
        Board project = prepareTestData();
        //When
        User user = new User("developer1", "John Smith");
        List<Task> taskList = project.getTaskLists().stream()
                .flatMap(l -> l.getTaskList().stream())
                .filter(t -> t.getAssignedUser().equals(user))
                .collect(toList());
        //Then
        Assert.assertEquals(2,taskList.size());
        Assert.assertEquals(user,taskList.get(0).getAssignedUser());
        Assert.assertEquals(user,taskList.get(1).getAssignedUser());
    }
    ...
    public void testAddTaskListFindOutdatedTasks(){
        ...
        List<Task> taskList = project.getTaskLists().stream()
                ...
                .flatMap(tl -> tl.getTaskList().stream())
                .filter(t -> t.getDeadLine().isBefore(LocalDate.now()))
                .collect(toList());
        //Then
        Assert.assertEquals(1, taskList.size());
        Assert.assertEquals("HQLs for analysis", taskList.get(0).getTitle());
    }
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        double averageTimePassedOnTasksInProgress = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTaskList().stream())
                .map(t -> t.getCreated())
                .map(date -> date.until(LocalDate.now(),ChronoUnit.DAYS))
                .mapToDouble(n -> n)
                .average().getAsDouble();
        //Then
        Assert.assertEquals(10.0,averageTimePassedOnTasksInProgress,0.001);
    }
}
