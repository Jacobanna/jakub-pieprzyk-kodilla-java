    ...
    public static class CheckersBuilder {
        ...
        public Checkers build() {
            if (playerOne.length() == 0) {
                throw new IllegalStateException("No player one specified.");
            }
            if (playerTwo.length() == 0) {
                throw new IllegalStateException("No player two specified.");
            }
            ...
            if(!(black && white)) {
                throw new IllegalStateException("All figures on board in one color.");
            }
